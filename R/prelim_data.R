...
# Ostensive vs. non-ostensive

ci_data <- read.csv(file = here("data/mm1_predicted_ci_for_conditionMD1_Vienna.csv"))

agg_data_noodor <- agg_data %>%
  filter(condition != "odour", condition != "w_2cp_vd")

agg_data_noodor$condition2 <- jitter(as.numeric(as.factor(agg_data_noodor$condition), amount = .0001))


md_plot_prereg <- ggplot(data = agg_data_noodor, aes(x = condition, y = mean.resp)) +
  geom_line(aes(x = condition2, group = subject_ID), color = "gray", alpha = 0.5) +
  geom_point(data = agg_data_noodor %>% filter(condition == "non"), aes(x = condition2), color = "dodgerblue", size = 1.5, alpha = .5) +
  geom_point(data = agg_data_noodor %>% filter(condition == "ost"), aes(x = condition2), color = "darkorange", size = 1.5, alpha = .5, ) +
  geom_half_violin(data = agg_data_noodor %>% filter(condition == "non"), aes(x = condition2, y = mean.resp), position = position_nudge(x = -.5), side = "l", width = .2, fill = "dodgerblue", alpha = .5) +
  geom_half_violin(data = agg_data_noodor %>% filter(condition == "ost"), aes(x = condition2, y = mean.resp), position = position_nudge(x = 0.5), side = "r", width = .2, fill = "darkorange", alpha = .5) +
  geom_errorbar(data = ci_data %>% filter(condition == "ost"), aes(x = 2.3, y = fitted, ymin = lower.cl, ymax = upper.cl), color = "darkorange", width = 0.1, size = 0.6) +
  geom_errorbar(data = ci_data %>% filter(condition == "non"), aes(x = 0.65, y = fitted, ymin = lower.cl, ymax = upper.cl), color = "dodgerblue", width = 0.1, size = 0.6) +
  geom_point(data = ci_data %>% filter(condition == "ost"), aes(x = 2.3, y = fitted), color = "darkorange", size = 1.5) +
  geom_point(data = ci_data %>% filter(condition == "non"), aes(x = 0.65, y = fitted), color = "dodgerblue", size = 1.5) +
...
od_plot_prereg <- ggplot(data = agg_data_odor, aes(x = as.numeric(condition), y = mean.resp)) +
  geom_jitter(data = agg_data_odor, aes(x = as.numeric(condition)), color = "grey", size = 1.5, alpha = .5, height = 0, width = 0.2) +
  geom_half_violin(data = agg_data_odor, aes(x = as.numeric(condition), y = mean.resp), position = position_nudge(x = -.3), side = "l", width = .2, fill = "grey", alpha = .5) +
  geom_hline(yintercept = 0.5, lty = 2, col = "red") +
  xlim(2, 2.5) +
  xlab("") +
  ylab("Proportion correct") +
...
md_plot_prereg + od_plot_prereg + 
  plot_layout(widths = c(2, 1)) + plot_annotation(tag_levels = 'A')

ggsave(here("graphs/md1_conditions.png"), width = 10, height = 8, scale = 0.5)
...
reli_data <- read_csv(file = here("data/md1_reli_data_vienna.csv"))
# view(dfSummary(reli.data))
reliability_kappa <- cohen.kappa(cbind(reli_data$response_orig,reli_data$response_reli))
